...
wc = spg_bpdn(G, y/sqrt(N), noise, opts_spgl1); 
...
plot(t, f_values_samp, 'color', cww_dflt.blue, 'linewidth', cww_dflt.line_width);
hold('on');
...
legend({'$x$', 'CS-infinite'}, 'location', 'northeast', 'fontsize', cww_dflt.font_size, ...
        'Interpreter','latex');
set(gca, 'FontSize', cww_dflt.font_size);

fname = sprintf('CS_1d_wavelet_fin_samples_N_%d_M_%d_%s_%s', N, M, wname, bd_mode);
if do_save
...
wc = spg_bpdn(G_fin, y, noise, opts_spgl1); 
sc = wl_idwt_impl_from_kernel(wc, idwt_kernel);
...
legend({'$x$', 'CS-finte'}, 'location', 'northeast', 'fontsize', cww_dflt.font_size, ...
        'Interpreter','latex');
set(gca, 'FontSize', cww_dflt.font_size);

fname = sprintf('CS_1d_wavelet_fin_samples_fin_wcrime_N_%d_M_%d_%s_%s', N, M, wname, bd_mode);
