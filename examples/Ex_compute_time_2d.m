...
% in .mat files. Use the script `Ex_create_time_plot_2d.m` to plot the result.
...
for i = 1:n
    ...
    M = 2^R;
    N = 2^(R+q);
%    X = randn(N*N, M*M);%cww_generate_full_matrix_1d(R+q, R, wname, bd_mode, j0);
...
fname = sprintf('time_2d_forward_R_%d_%d_q_%d_db%d_%s_j0_%d.mat', R_range(1), R_range(end), q, vm, bd_mode, j0);
...
for i = 1:n
    ...
    M = 2^R;
    N = 2^(R+q);
    %X = randn(M*M, N*N);%cww_generate_full_matrix_1d(R+q, R, wname, bd_mode, j0);
    phi_walsh_pieces = cww_get_phi_walsh_pieces(R+q, R, wname, bd_mode, j0);
    G = @(x,mode) cww_handle_2d(x, mode, R+q, R, wname, bd_mode, j0, phi_walsh_pieces); 
    x = randn(N*N,1);
    xi = randn(N,N);
...
fname = sprintf('time_2d_adjoint_R_%d_%d_q_%d_db%d_%s_j0_%d.mat', R_range(1), R_range(end), q, vm, bd_mode, j0);
