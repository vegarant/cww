...
%
% sph2_rect creates a rectangular sampling pattern for an image of size N = 2^R
%  +-----------------------------------+
%  |  |    |        |                  |
%  |--+    |        |                  |
%  |       |        |                  |
...
%
function [idx, scales] = sph2_rect(N, nbr_samples)
...
% This is a support function for `sph2_rect` and `sch2_rect`.
...
function scheme = rect_samp_scheme(N, nbr_samples)
    ...
    if (2^nu ~= N)
        error('cslib:value_error', 'Image size must be power of 2');
    end 
    m = ones(nu,1); 
    i=2;
